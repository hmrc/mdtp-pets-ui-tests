...
cd $REPO
npm install
echo y | PORT=6001 npm run dev >> ../../sbt-report 2>&1 & # The ampersand (&) is needed to run this task in the background and the echo y is to skip through an analytics question
